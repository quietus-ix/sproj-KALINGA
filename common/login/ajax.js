      ...
      $.ajax({
         ...
         success: function(response){
            if(response.statusCode==400) {
               $('#si_notice').parent().removeClass('tw-hidden');
               $('#si_notice').parent().addClass('tw-flex shake');
               $('#si_notice').html(response.message);
               setTimeout(()=>{
                  $('#si_notice').parent().removeClass('tw-flex shake');
                  $('#si_notice').parent().addClass('tw-hidden');
               },3000);
            }
      ...
      $.ajax({
         ...
         success: function(response){
            if(response.statusCode==400 && response.type==1) {
               $('#su_notice').parent().removeClass('tw-hidden');
               $('#su_notice').parent().addClass('tw-flex shake');
               $('#su_notice').html(response.message);
               setTimeout(()=>{
                  $('#su_notice').parent().removeClass('tw-flex shake');
                  $('#su_notice').parent().addClass('tw-hidden');
               },3000);
            }
            else if(response.statusCode==400 && response.type==2) {
               $('#un_msg').html(response.message);
               $('#SU_username').parent().toggleClass('tw-border-tertiary', 'tw-border-err');
               ...
               $('#pw_msg').removeClass('tw-opacity-0');
               $('#pw_msg').html(response.message);
               $('#SU_password').parent().toggleClass('tw-border-tertiary', 'tw-border-err');
               ...
               $('#pw_msg, #rpw_msg').removeClass('tw-opacity-0 tw-text-wrng');
               $('#pw_msg, #rpw_msg').addClass('tw-text-err');
               $('#pw_msg').html(response.message);
               $('#rpw_msg').html(response.message);
               $('#SU_password, #SU_repassword').parent().parent().removeClass('tw-border-tertiary tw-border-wrng');
               $('#SU_password, #SU_repassword').parent().parent().addClass('shakeH tw-border-err');
               setTimeout(()=>{
