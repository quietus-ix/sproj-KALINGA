export function loadTableContent(target, src, table, column, colSel) { // colToGrp
	$.ajax({
		url: src,
		method: "GET",
		success: function (data) {
			...
			new DataTable(table, {
				...
				// columnDefs: [{ visible: false, targets: colToGrp }],
				// order: [[colToGrp, 'asc']],
				// displayLength: 25,
				...
				// 	api.column(colToGrp, { page: 'current' }).data().each(function (group, i) {
				// 		if (last !== group) {
				// 			$(rows)
				// 			.eq(i)
				// 			.before(
				...
				initComplete: function () {
					this.api().columns(column).every(function () {
						let column = this;
						...
						select.addEventListener("change", function () {
							let val = DataTable.util.escapeRegex(
								select.value
							);
...
$(document).ready(() => {
	...
	$("#nav_inventory").click(() => {
		...
		loadTableContent("main", "inventory.php", "#inv_tbl", [1], [0, 1, 2, ':visible']); // add 1
	});
